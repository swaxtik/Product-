class GalleryManager {
    constructor() {
        ...
        this.isProductFiltersExpanded = false;
        this.activeFilters = {
            weightRange: 'all',
            metalType: 'all',
            gst: 'all',
    ...
    setupEventListeners() {
        ...
        const customWeightMin = document.getElementById('customWeightMin');
        const customWeightMax = document.getElementById('customWeightMax');
        ...
        weightFilter.addEventListener('change', (e) => {
            this.activeFilters.weightRange = e.target.value;
            this.toggleCustomWeightInputs();
            this.renderGallery(this.currentImages);
        });
        metalFilter.addEventListener('change', (e) => {
            this.activeFilters.metalType = e.target.value;
            this.renderGallery(this.currentImages);
        });
        gstFilter.addEventListener('change', (e) => {
            this.activeFilters.gst = e.target.value;
            this.renderGallery(this.currentImages);
        });
        productSearch.addEventListener('input', (e) => {
            this.activeFilters.search = String(e.target.value || '').trim().toLowerCase();
            this.renderGallery(this.currentImages);
        });
        clearProductFilters.addEventListener('click', () => {
            this.resetProductFilters(true);
            this.renderGallery(this.currentImages);
        });
        toggleProductFilters.addEventListener('click', () => {
            this.setFilterPanelExpanded(!this.isProductFiltersExpanded);
        });
        customWeightMin.addEventListener('input', (e) => {
            this.activeFilters.customWeightMin = String(e.target.value || '').trim();
            this.renderGallery(this.currentImages);
        });
        customWeightMax.addEventListener('input', (e) => {
            this.activeFilters.customWeightMax = String(e.target.value || '').trim();
            this.renderGallery(this.currentImages);
        });
    ...
    showCategoriesView() {
        ...
        this.categoryScreenMode = 'subcategories';
        this.resetProductFilters(true);
        this.updateProductFilterVisibility(false);
    }
    ...
    showProductsForCurrentSubcategory() {
        ...
        this.updateProductFilterVisibility(true);
    ...
    renderGallery(images) {
        ...
        const subcategoryImages = this.currentSubcategory === this.uncategorizedId
            ? images.filter((image) => !image.subcategoryId)
            : images.filter((image) => image.subcategoryId === this.currentSubcategory);
        this.updateProductFilterOptions(subcategoryImages);
        const filteredImages = this.applyProductFilters(subcategoryImages);
        this.filteredImages = filteredImages;

        if (filteredImages.length === 0) {
            gallery.innerHTML = '';
            if (subcategoryImages.length > 0) {
                emptyTitle.textContent = 'No products match selected filters';
                emptyText.textContent = 'Try a different weight range, metal type, GST, or search text.';
            } else {
                emptyTitle.textContent = 'No items in this collection';
                emptyText.textContent = 'Add some beautiful jewelry pieces to showcase';
            }
            emptyState.classList.remove('hidden');
            return;
        }
        ...
        gallery.innerHTML = filteredImages.map((image, index) => `
            <div class="gallery-item" data-index="${index}" data-id="${image.id}">
                ${(image.thumbnail || image.data)
                    ...
                    : `<div class="gallery-item-image-placeholder">${image.tagNumber || 'No Image'}</div>`
                }
                <div class="gallery-item-info">
                    <div class="gallery-item-title">${image.title}</div>
                    <div class="gallery-item-actions">
                        <button class="btn btn-primary view-image-btn" type="button" ${(image.thumbnail || image.data) ? '' : 'disabled'}>${(image.thumbnail || image.data) ? 'View Image' : 'No Image Yet'}</button>
        ...
        gallery.querySelectorAll('.gallery-item').forEach(item => {
            ...
            const hasImage = Boolean(currentImage.thumbnail || currentImage.data);
    ...
    updateProductFilterVisibility(shouldShow) {
        const filterBar = document.getElementById('productFilterBar');
        const toggleButton = document.getElementById('toggleProductFilters');
        if (!filterBar || !toggleButton) return;
        toggleButton.classList.toggle('hidden', !shouldShow);
        if (!shouldShow) {
            this.setFilterPanelExpanded(false);
        } else {
            filterBar.classList.toggle('hidden', !this.isProductFiltersExpanded);
        }
    }

    setFilterPanelExpanded(expanded) {
        this.isProductFiltersExpanded = expanded;
        const filterBar = document.getElementById('productFilterBar');
        const toggleButton = document.getElementById('toggleProductFilters');
        if (filterBar) {
            filterBar.classList.toggle('hidden', !expanded);
        }
        if (expanded) {
            this.toggleCustomWeightInputs();
        }
        if (toggleButton) {
            toggleButton.textContent = expanded ? 'Hide Filters' : 'Show Filters';
            toggleButton.setAttribute('aria-expanded', expanded ? 'true' : 'false');
        }
    }

    resetProductFilters(updateUI = false) {
        this.activeFilters = {
            weightRange: 'all',
            metalType: 'all',
            gst: 'all',
        ...
        if (gstFilter) gstFilter.value = 'all';
        if (productSearch) productSearch.value = '';
        if (customWeightMin) customWeightMin.value = '';
        if (customWeightMax) customWeightMax.value = '';
        this.toggleCustomWeightInputs();
    }

    updateProductFilterOptions(subcategoryImages) {
        const metalFilter = document.getElementById('metalFilter');
        const gstFilter = document.getElementById('gstFilter');
        if (!metalFilter || !gstFilter) {
            return;
        }

        const metalTypes = this.getDistinctFilterValues(subcategoryImages, 'goldType');
        const gstValues = this.getDistinctFilterValues(subcategoryImages, 'gst');

        const currentMetal = this.activeFilters.metalType;
        const currentGst = this.activeFilters.gst;

        metalFilter.innerHTML = '<option value="all">All Metals</option>' + metalTypes
            .map((value) => `<option value="${value.normalized}">${value.display}</option>`)
            .join('');
        gstFilter.innerHTML = '<option value="all">All GST</option>' + gstValues
            .map((value) => `<option value="${value.normalized}">${value.display}</option>`)
            .join('');

        const metalExists = currentMetal === 'all' || metalTypes.some((item) => item.normalized === currentMetal);
        const gstExists = currentGst === 'all' || gstValues.some((item) => item.normalized === currentGst);

        this.activeFilters.metalType = metalExists ? currentMetal : 'all';
        this.activeFilters.gst = gstExists ? currentGst : 'all';
        metalFilter.value = this.activeFilters.metalType;
        gstFilter.value = this.activeFilters.gst;
    }

    getDistinctFilterValues(images, field) {
        const values = new Map();
        for (const image of images) {
            const raw = String(image[field] || '').trim();
            if (!raw) continue;
            const normalized = raw.toLowerCase();
            if (!values.has(normalized)) {
                values.set(normalized, raw);
            }
        }
    ...
    applyProductFilters(images) {
        const { weightRange, metalType, gst, search, customWeightMin, customWeightMax } = this.activeFilters;

        return images.filter((image) => {
            if (weightRange !== 'all') {
                const weightInGrams = this.extractWeightInGrams(image.weight);
                const matchesWeight = weightRange === 'custom'
                    ? this.matchesCustomWeightRange(weightInGrams, customWeightMin, customWeightMax)
                    : this.matchesWeightRange(weightInGrams, weightRange);
                if (!matchesWeight) {
                    return false;
                }
            }
            ...
            if (metalType !== 'all') {
                const imageMetal = String(image.goldType || '').trim().toLowerCase();
                if (imageMetal !== metalType) {
                    return false;
                }
            }

            if (gst !== 'all') {
                const imageGst = String(image.gst || '').trim().toLowerCase();
                if (imageGst !== gst) {
                    return false;
                }
            }
            ...
            if (search) {
                const searchText = `${image.tagNumber || ''} ${image.title || ''}`.toLowerCase();
    ...
    toggleCustomWeightInputs() {
        const customWeightRange = document.getElementById('customWeightRange');
        if (!customWeightRange) {
            return;
        }
        const shouldShow = this.activeFilters.weightRange === 'custom';
        customWeightRange.classList.toggle('hidden', !shouldShow);
    }

    extractWeightInGrams(weightValue) {
        if (!weightValue) return null;
        const match = String(weightValue).replace(',', '.').match(/(\d+(\.\d+)?)/);
        return match ? parseFloat(match[1]) : null;
    }

    matchesWeightRange(weightInGrams, range) {
        if (weightInGrams === null || Number.isNaN(weightInGrams)) {
            return false;
        }

        if (range === '1-5') {
            return weightInGrams >= 1 && weightInGrams <= 5;
        }
        if (range === '5-10') {
            return weightInGrams > 5 && weightInGrams <= 10;
        }
        if (range === '10-30') {
            return weightInGrams > 10 && weightInGrams <= 30;
        }
        if (range === '30+') {
            return weightInGrams > 30;
        }
        return true;
    }

    matchesCustomWeightRange(weightInGrams, minValue, maxValue) {
        ...
        const min = hasMin ? parseFloat(minValue) : null;
        const max = hasMax ? parseFloat(maxValue) : null;
        if (min !== null && !Number.isNaN(min) && weightInGrams < min) {
            return false;
        }
        if (max !== null && !Number.isNaN(max) && weightInGrams > max) {
            return false;
        }
        return true;
    }
    ...
    openDetailsModal(image) {
        ...
        if (image.thumbnail || image.data) {
            detailsImage.src = image.thumbnail || image.data;
