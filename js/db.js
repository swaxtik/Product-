class DatabaseManager {
    ...
    async getImageByCategoryAndTag(categoryId, tagNumber) {
        ...
        return images.find((image) => this.normalizeTagNumber(image.tagNumber) === normalizedTag) || null;
    }

    async upsertImageByTagInCategory(imageData) {
        const normalizedTag = this.normalizeTagNumber(imageData.tagNumber);
        const categoryId = imageData.category;
        const existing = normalizedTag && categoryId
            ? await this.getImageByCategoryAndTag(categoryId, normalizedTag)
            : null;
        ...
        if (existing) {
            return this.updateImage(existing.id, payload);
        }

        return this.addImage(payload);
    }
