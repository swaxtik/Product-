class AdminManager {
    ...
    resolveCategoryIdFromRow(rawCategory, categories, fallbackCategoryId = '') {
        const input = String(rawCategory || '').trim();
        if (!input) return fallbackCategoryId;

        const normalizedInput = this.normalizeText(input);
        const exactId = categories.find((category) => category.id === input);
        if (exactId) return exactId.id;

        const idMatch = categories.find((category) => this.normalizeText(category.id) === normalizedInput);
        if (idMatch) return idMatch.id;

        const nameMatch = categories.find((category) => this.normalizeText(category.name) === normalizedInput);
        if (nameMatch) return nameMatch.id;

        return '';
    }

    async createOrUpdatePlaceholderFromMetadata(row, categories, fallbackCategoryId = '') {
        const tagNumber = this.normalizeTagNumber(row.tagNumber);
        if (!tagNumber) {
            return { status: 'skipped' };
        }

        const categoryId = this.resolveCategoryIdFromRow(row.category, categories, fallbackCategoryId);
        if (!categoryId) {
            return { status: 'missingCategory' };
        }

        const subCategoryName = String(row.subCategory || '').trim();
        let resolvedSubcategory = null;
        if (subCategoryName) {
            resolvedSubcategory = await window.dbManager.ensureSubcategory(categoryId, subCategoryName);
        }

        const title = String(row.title || '').trim() || tagNumber;
        const description = String(row.description || '').trim();
        const price = String(row.price || '').trim();
        const goldType = String(row.goldType || '').trim();
        const weight = String(row.weight || '').trim();
        const gst = String(row.gst || '').trim();
        const existing = await window.dbManager.getImageByCategoryAndTag(categoryId, tagNumber);

        await window.dbManager.upsertImageByTagInCategory({
            tagNumber,
            title,
            description,
            price,
            ...
            type: existing?.type || 'metadata-placeholder',
            fileName: existing?.fileName || '',
            isPlaceholder: !(existing?.data || existing?.thumbnail)
        });
    ...
    async importMetadataFile() {
        ...
        try {
            ...
            const categories = await window.dbManager.getCategories();
            const fallbackCategoryId = window.gallery.currentCategory || '';
            ...
            for (const row of normalizedRows) {
                const result = await this.createOrUpdatePlaceholderFromMetadata(row, categories, fallbackCategoryId);
                if (result.status === 'created') placeholderStats.created++;
                if (result.status === 'updated') placeholderStats.updated++;
                if (result.status === 'missingCategory') placeholderStats.missingCategory++;
            }

            this.showSuccess(
                `Metadata imported: ${imported}, skipped: ${skipped}. Products shown now: ${placeholderStats.created + placeholderStats.updated}. Missing category: ${placeholderStats.missingCategory}.`
            );
    ...
    async uploadImages() {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                await window.dbManager.upsertImageByTagInCategory(imageRecord);
                uploadedCount++;
